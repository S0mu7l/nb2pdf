...
**exe download / exe 下载** (v1.2.0 on [GitHub Releases](https://github.com/S0mu7l/nb2pdf/releases)):

- 中文界面：`nb2pdf-zh-windows-1.2.0.rar` → extract `nb2pdf-zh.exe` / 解压得 `nb2pdf-zh.exe`
- English UI: `nb2pdf-en-windows-1.2.0.rar` → extract `nb2pdf-en.exe` / 解压得 `nb2pdf-en.exe`

Both editions support Chinese/CJK file paths — pick either one. / 两个版本均支持中文路径，按需下载其一即可。
